...
struct pie_params {
	...
	bool max_prob;		/* true to deterministically drop the packets in madpie */
...
static int pie_change(struct Qdisc *sch, struct nlattr *opt)
{
	...
	if (tb[TCA_PIE_MADPIE])
		q->params.madpie = nla_get_u32(tb[TCA_PIE_MADPIE]);
...
static void calculate_probability(struct Qdisc *sch)
{
    ...
    /* Update max_prob
     * if madpie is set and current delay is greater than hard_delay
     */
...
static int pie_dump(struct Qdisc *sch, struct sk_buff *skb)
{
	...
	if (nla_put_u32(skb, TCA_PIE_TARGET,
	    ...
	    nla_put_u32(skb, TCA_PIE_MADPIE, q->params.madpie) ||
