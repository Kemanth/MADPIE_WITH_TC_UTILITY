...
static void explain(void)
{
	...
	fprintf(stderr, "[ madpie ]      [ tupdate TIME us][ alpha ALPHA ]");
			 ...
			 struct nlmsghdr *n)
{
	...
	if (madpie)
		addattr_l(n, 1024, TCA_PIE_MADPIE, &madpie, sizeof(madpie));
	if (ecn != -1)
...
static int pie_print_opt(struct qdisc_util *qu, FILE *f, struct rtattr *opt)
{
	...
	if (tb[TCA_PIE_MADPIE] && RTA_PAYLOAD(tb[TCA_PIE_MADPIE]) >= sizeof(__u32)) {
		madpie = rta_getattr_u32(tb[TCA_PIE_MADPIE]);
		if (madpie)
			fprintf(f, "madpie ");
	}
